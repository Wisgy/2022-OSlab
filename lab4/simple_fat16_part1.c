...
{ 
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      /*** BEGIN ***/
      BYTE file_name[12]={0};
      if(!(sector_buffer[((i-1)%16)*32+11]==0x0f||sector_buffer[((i-1)%16)*32]==0||sector_buffer[((i-1)%16)*32]==0xe5)){
        for(uint j=0;j<11;j++){
          file_name[j]=sector_buffer[((i-1)%16)*32+j];
        }
        BYTE *F_name=path_decode(file_name);
        if(F_name[0]!=0){
          // printf("filename=%s\n-----i=%d\n",F_name,i);
          filler(buffer,F_name,NULL,0);
        }
      }
      /*** END ***/
               ...
               struct fuse_file_info *fi)
{
  ...
  size_t end_off;
  ClusterN = Dir.DIR_FstClusLO;
  int sec_num=fat16_ins->Bpb.BPB_SecPerClus;//每个簇的扇区数量
  while(1){//查找目录
    if((offCluster+1)*sec_num*BYTES_PER_SECTOR>beginBytes&&offCluster*sec_num*BYTES_PER_SECTOR<=beginBytes){
      beginCluster=ClusterN;
      for(;beginSecCnt*BYTES_PER_SECTOR+offCluster*sec_num*BYTES_PER_SECTOR<=beginBytes;){
        beginSecCnt++; 
      }
      begin_off=beginBytes-(beginSecCnt-1)*BYTES_PER_SECTOR-offCluster*sec_num*BYTES_PER_SECTOR;
    }
    if((offCluster+1)*sec_num*BYTES_PER_SECTOR>endBytes&&offCluster*sec_num*BYTES_PER_SECTOR<=endBytes){
      endCluster=ClusterN;
      for(;endSecCnt*BYTES_PER_SECTOR+offCluster*sec_num*BYTES_PER_SECTOR<=endBytes;){
        endSecCnt++;
      }
      break;
    }
    ...
    // printf("nextClusterN=%x\n",ClusterN);
    if(ClusterN==0xffff){
      while(endSecCnt*BYTES_PER_SECTOR+offCluster*sec_num*BYTES_PER_SECTOR<=endBytes){
        endSecCnt++; 
      }
      break;
    }
  ...
  end_off=endBytes-(endSecCnt-1)*BYTES_PER_SECTOR-offCluster*sec_num*BYTES_PER_SECTOR;
  if(beginCluster==endCluster){
    BYTE sector_buffer[BYTES_PER_SECTOR]={0};
    first_sector_by_cluster(fat16_ins, beginCluster, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
    printf("cluster=%d---sector=%d\n",beginCluster,FirstSectorofCluster);
    if(beginSecCnt==endSecCnt){
      memcpy(buffer,&sector_buffer[begin_off],end_off-begin_off+1);
      return size;
    }
    else{
      memcpy(buffer,&sector_buffer[begin_off],512-begin_off);
      size_t next=512-begin_off;
      beginSecCnt++;
      while(beginSecCnt<endSecCnt){
        sector_read(fat16_ins->fd,FirstSectorofCluster+beginSecCnt-1,sector_buffer);
        memcpy(&buffer[next],&sector_buffer,BYTES_PER_SECTOR);
        next+=512;
        beginSecCnt++;
      }
      memcpy(&buffer[next],&sector_buffer,end_off+1);
      return next+end_off+1;
    }
    printf("%s",buffer);
  }
  BYTE sector_buffer[BYTES_PER_SECTOR]={0};
  ...
  size_t next=512-begin_off;
  // printf("copybegin\n");
  while(beginSecCnt<sec_num){
    beginSecCnt++;
    sector_read(fat16_ins->fd,FirstSectorofCluster+beginSecCnt-1,sector_buffer);
    memcpy(&buffer[next],&sector_buffer,BYTES_PER_SECTOR);
    next+=512;
  }
  ClusterN=fat_entry_by_cluster(fat16_ins,beginCluster);
  // printf("copymiddle\n");
  while(ClusterN!=endCluster){
    int SecCnt=0;
    first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
    while(SecCnt<sec_num){
      SecCnt++;
      // printf("seccnt=%d\n",SecCnt);
      sector_read(fat16_ins->fd,FirstSectorofCluster+SecCnt-1,sector_buffer);
      memcpy(&buffer[next],&sector_buffer,BYTES_PER_SECTOR);
      next+=512;
    }
    ClusterN=fat_entry_by_cluster(fat16_ins,ClusterN);
  }
  first_sector_by_cluster(fat16_ins, endCluster, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
  int SecCnt=0;
  // printf("copyend\n");
  // printf("%d----%d\n",endSecCnt,beginSecCnt);
  while(SecCnt<endSecCnt){
    SecCnt++;
    // printf("seccnt=%d\n",SecCnt);
    sector_read(fat16_ins->fd,FirstSectorofCluster+SecCnt-1,sector_buffer);
    if(SecCnt==endSecCnt){
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  if (findFlag == 1) {
    ...
    /*** BEGIN ***/
    // printf("\n\n\n\nI create it %d,%d,%s\n\n\n\n\n",sectorNum,offset,paths[pathDepth-1]);
    // printf("sector=%d\n",sectorNum);
    dir_entry_create(fat16_ins,sectorNum,offset,paths[pathDepth-1],0x20,0xffff,0);
    /*** END ***/
  }
  return 0;
}
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  WORD tmp=0x0000;
  memcpy(&sector_buffer[FatEntOffset],&tmp,2*sizeof(BYTE));
  sector_write(fd,FatSecNum,sector_buffer);
  FatSecNum = fat16_ins->Bpb.BPB_RsvdSecCnt+ fat16_ins->Bpb.BPB_FATSz16 + (FATOffset / fat16_ins->Bpb.BPB_BytsPerSec);
  sector_read(fd, FatSecNum, sector_buffer);
  memcpy(&sector_buffer[FatEntOffset],&tmp,2*sizeof(BYTE));
  sector_write(fd,FatSecNum,sector_buffer);
  /*** END ***/
...
int fat16_unlink(const char *path)
{
  ...
  if (findFlag == 1)
  {
    ...
    sector_read(fat16_ins->fd,sectorNum,sector_buffer);
    uint8_t tmp1=0xe5;
    memcpy(&sector_buffer[offset],&tmp1,1*sizeof(BYTE));
    sector_write(fat16_ins->fd,sectorNum,sector_buffer);
    /*** END ***/
  }
  return 0;
