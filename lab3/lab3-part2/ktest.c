...
// 定义要输出的文件
#define OUTPUT_FILE "/home/wisgy/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff846b03f0;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff84693dd0;
...
static void print_mm_active_info(void)
{
    ...
    if(mm){
        ...
        for(;vma!=NULL;vma=vma->vm_next){
            ...
            for(;virt_addr<=vma->vm_end;virt_addr+=4096){
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                if(IS_ERR_OR_NULL(page)!=1){
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    unsigned long pgdt=mm->pgd;
    pgd_t* pgdoff=pgd_offset(mm,virt);
    pud_t* pudoff=pud_offset((p4d_t *)pgdoff,virt);
    pmd_t* pmdoff=pmd_offset(pudoff,virt);
    pte_t* pteoff=pte_offset_kernel(pmdoff,virt);
    page=pte_page(__pte(pteoff));
    if (page)
    {   
...
static void print_seg_info(void)
{
    ...
    unsigned long begin=mm->start_data;
    unsigned long end=mm->end_data;
    unsigned long begin2=mm->start_code;
    unsigned long end2=mm->end_code;
    unsigned long address=begin;
    struct vm_area_struct *vma=find_vma(mm,address);
    for(;address<=end;address+=PAGE_SIZE){
        struct vm_area_struct *vma=find_vma(mm,address);
        struct page *page =mfollow_page(vma, address, FOLL_GET);
        if(IS_ERR_OR_NULL(page)!=1){
            char * addr=kmap_atomic(page);
            kernel_write(log_file, addr, PAGE_SIZE, &log_file->f_pos);
            kunmap_atomic(addr);
        }
    }
    address=begin2;
    for(;address<=end2;address+=PAGE_SIZE){
        struct vm_area_struct *vma=find_vma(mm,address);
        struct page *page =mfollow_page(vma, address, FOLL_GET);
        if(IS_ERR_OR_NULL(page)!=1){
            char * addr=kmap_atomic(page);
